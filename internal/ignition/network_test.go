...
func TestInjectStaticNetwork(t *testing.T) {
	...
	for _, f := range files {
		...
		if fm["path"] == enp1s0NMConnectionPath {
			...
			if !strings.Contains(decoded, "method=manual") {
				t.Errorf("expected method=manual, got:\n%s", decoded)
			}
			if !strings.Contains(decoded, "interface-name=enp1s0") {
				t.Errorf("expected interface-name=enp1s0, got:\n%s", decoded)
			}
			if !strings.Contains(decoded, "type=ethernet") {
				t.Errorf("expected type=ethernet, got:\n%s", decoded)
			}
	...
	if !found {
		t.Error("expected enp1s0.nmconnection file in ignition output")
	}
}
...
func TestInjectStaticNetworkDNS(t *testing.T) {
	input := testIgnitionV3Input
	net := NetworkConfig{
		Addresses:  []string{"192.168.122.100/24"},
	...
	for _, f := range files {
		...
		if fm["path"] == enp1s0NMConnectionPath {
			...
			source := contents["source"].(string)
			decoded, _ := url.PathUnescape(strings.TrimPrefix(source, "data:,"))
			if !strings.Contains(decoded, "dns=8.8.8.8;8.8.4.4;") {
				t.Errorf("expected dns servers, got:\n%s", decoded)
			}
			if !strings.Contains(decoded, "dns-search=example.com;") {
				t.Errorf("expected dns-search, got:\n%s", decoded)
			}
			return
		}
	}
